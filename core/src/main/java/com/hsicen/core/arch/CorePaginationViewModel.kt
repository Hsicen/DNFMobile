...
abstract class CorePaginationViewModel<T>(
  ...
  fun getPaginationData(isLoadMore: Boolean = false) = launch {
    ...
    // 先从缓存中读取
    paginationActionFromCache(pageSize, localPageNumber)
      ?.onSuccess {
        // 更新页数
...
fun <T : Any> CorePaginationViewModel.PaginationState<T>.applyTo(swipeRefreshLayout: SmartRefreshLayout) {
  // 如果是缓存读取状态不取消加载状态
